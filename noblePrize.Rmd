...
medicine_names <- nobels$laureates %>%
  unnest(prizes) %>%
  filter(category == "medicine", as.numeric(year) > 1900) %>%
...
medicine_names
head(medicine_names)
author_names <- medicine_names$full_name
...
write.csv(author_id_map, "author_id_map_noble.csv", row.names = FALSE)
write.csv(author_ids, "author_ids_noble.csv", row.names = FALSE)
author_ids <- read.csv("author_ids_noble.csv")
author_id_map <- read.csv("author_id_map_noble.csv")
```
...
author_ids <- dplyr::pull(author_ids, x)
all_papers <- list()

for(aid in author_ids){
  aid <- author_ids[i]
     
     cat("\n[", i, "] Fetching papers for:", aid, "\n")
     
     papers <- tryCatch(
         get_all_papers(aid),
         error = function(e){
             cat("Error at index", i, ":", aid, "\n")
             return(NULL)
         }
     )
...
edges_weighted <- edges_df %>%
  dplyr::group_by(author1, author2) %>%
...
# ================================
write_graph(g, file = "coauthorship_network_noble.gml", format = "gml")
```
...
write.csv(netSummary, "netSummary_noble.csv", row.names = FALSE)
```
...
write.csv(top3_per_community, "top3_per_community_noble.csv", row.names = FALSE)
```
...
# Save
ggsave("coauthorship_plot_noble.png", plot = p, width = 8, height = 6)
p
```
...
plotly_plot
htmlwidgets::saveWidget(plotly_plot, "interactive_plot_noble.html", selfcontained = TRUE)
```
...
# Save as HTML
saveWidget(fn, "network_noble.html", selfcontained = TRUE)
browseURL("network_noble.html")
